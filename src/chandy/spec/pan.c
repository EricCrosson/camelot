...
short src_ln1 [] = {
	  0,  29,  30,  31,  32,  34,  35,  36, 
	 33,  38,  38,  39,  40,  41,  28,  44, 
	 28,  44,  45,  46,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "chandy.pml", 1, 19 },
...
iniglobals(int calling_pid)
{
		...
		now.lastSent = 0;
		now.lastReceived = 0;
		now.messageAtRecord = 0;
		now.messageAtMarker = 0;
		now.ch = addqueue(calling_pid, 1, 0);
#ifdef VAR_RANGES
		logval("recorded", now.recorded);
		logval("lastSent", now.lastSent);
		logval("lastReceived", now.lastReceived);
		logval("messageAtRecord", now.messageAtRecord);
		logval("messageAtMarker", now.messageAtMarker);
#endif
}
...
#endif
	switch (n) {
	case 1: j = sizeof(Q1); q_flds[1] = 2; q_max[1] = 4; break;
...
{	/* int i; */
	...
	c_chandump(now.ch);
	printf("	byte   lastSent:	%d\n", now.lastSent);
	printf("	byte   lastReceived:	%d\n", now.lastReceived);
	printf("	byte   messageAtRecord:	%d\n", now.messageAtRecord);
	printf("	byte   messageAtMarker:	%d\n", now.messageAtMarker);
	printf("	bit    recorded:	%d\n", now.recorded);
}
void
...
{
	switch (x) {
	case 1: Printf("marker"); break;
	case 2: Printf("message"); break;
	default: Printf("%d", x);
	}
}
void
